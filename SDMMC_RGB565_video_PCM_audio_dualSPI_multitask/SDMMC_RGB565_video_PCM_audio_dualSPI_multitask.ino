  ...
  while (1)
  {
    if (showed_buffer_idx != loaded_buffer_idx)
    {
      ...
      if (buf)
      {
        ...
        gfx->startWrite();
        gfx->writeBytes(buf, RGB565_BUFFER_SIZE);
        gfx->endWrite();
    ...
    else
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            vBuf1 = (uint8_t *)malloc(RGB565_BUFFER_SIZE);
            if (!vBuf1)
            {
            ...
            {
              vBuf2 = (uint8_t *)malloc(RGB565_BUFFER_SIZE);
              if (!vBuf2)
              {
              ...
              // Play audio
                ...
                Serial.printf("SDMMC read RGB565: %d ms (%0.1f %%)\n", total_read_video, 100.0 * total_read_video / time_used);
                ...
                gfx->printf("SDMMC read RGB565: %d ms (%0.1f %%)\n", total_read_video, 100.0 * total_read_video / time_used);
